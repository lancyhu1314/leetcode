...
@Repository
public class Lns244 extends WorkUnit {
	...
	public void run() throws Exception {
		...
		if(!ConstantDeclare.PRIORTYPE.GDCINT.equals(specifiedType) && 
		   !ConstantDeclare.PRIORTYPE.NINT.equals(specifiedType) &&
		   !ConstantDeclare.PRIORTYPE.PRIN.equals(specifiedType) &&
		   !ConstantDeclare.PRIORTYPE.INTDINT.equals(specifiedType) ) 
...
//		LoggerUtil.debug("REPAYAMT1:" + getRepayAmt().getVal()); // 顾客上送还款金额
		...
		{
			//非标还款 增加还款方式13
			if(!("2512634".equals(lnsbasicinfo.getPrdcode())&&ConstantDeclare.REPAYWAY.REPAYWAY_YBYX.equals(lnsbasicinfo.getRepayway()))){
		...
		{
			//无追保理和买方付息产品的债务公司必输
			if(  ("3".equals(specifiedType) && Arrays.asList("3010006","3010013","3010014","3010015").contains(lnsbasicinfo.getPrdcode())) )
			{
...
//		FundInvest	fundInvest = new FundInvest("", "", repayChannel, bankSubject, outSerialNo);
		...
		if (ConstantDeclare.LOANSTATUS.LOANSTATUS_LANGUISHMENT.equals(lnsbasicinfo.getLoanstat())
			...
			if( ConstantDeclare.PRIORTYPE.INTDINT.equals(specifiedType)  )
		...
		for (TblLnsbill lnsbill : billList) {
			...
			if( (!ConstantDeclare.PRIORTYPE.NINT.equals(specifiedType) &&
				!ConstantDeclare.PRIORTYPE.INTDINT.equals(specifiedType) ) && 
			...
			if( !ConstantDeclare.PRIORTYPE.PRIN.equals(specifiedType) &&
...
//			}	
		...
		if (!"3".equals(endFlag) && getRepayAmt().isPositive()
			...
			if(ConstantDeclare.PRIORTYPE.NINT.equals(specifiedType) || ConstantDeclare.PRIORTYPE.INTDINT.equals(specifiedType))
				prePayFlag = ConstantDeclare.PREPAYFLAG.PREPAYSPEC_ONLYINT;
			if(ConstantDeclare.PRIORTYPE.PRIN.equals(specifiedType))
				prePayFlag = ConstantDeclare.PREPAYFLAG.PREPAYSPEC_ONLYPRIN;
			
			endFlag = LoanInterestSettlementUtil.specialOrderNonStd(loanAgreement, ctx, subNo, lnsbasicinfo, 
					new FabAmount(getRepayAmt().getVal()), prinAdvancelist, nintAdvancelist, billStatistics,prePayFlag);
...
//				reparam.put("debtAmt", debtAmt);
				...
				{	
					...
					if( debtAmt.isPositive() )
						eventProvider.createEvent(ConstantDeclare.EVENT.BACKDEBTCO, debtAmt, lnsAcctInfo, null, fundInvest1, "HKCT", ctx, preaccountInfo_N.getCustomid(),
				...
				{	
					...
					if( debtAmt.isPositive() )
					{
						...
						eventProvider.createEvent(ConstantDeclare.EVENT.BACKDEBTCO, debtAmt, lnsAcctInfo, null, loanAgreement.getFundInvest(), "HKCT", ctx, preaccountInfo_N.getCustomid(),
								debtCompany, ConstantDeclare.ACCOUNT.CUSTOMERTYPE.COMPANY);
					}
				}
