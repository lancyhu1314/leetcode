...
@Scope("prototype")
@Repository
public class Lns245 extends WorkUnit {
	...
	public void run() throws Exception {
		...
		if(VarChecker.isEmpty(channelType)){
			throw new FabException("LNS067");
		}
		
		if(VarChecker.isEmpty(chargeType)){
			throw new FabException("LNS068");
		}
		...
		//预收账号
		if(chargeType.equals("TXCT")){
			lnsinterface.setUserno("2");
		}else if(chargeType.equals("YBCT")){
			lnsinterface.setUserno("3");
		}else if(chargeType.equals("QBCT")){
		...
		try {
			lnsdiscountaccount = DbAccessUtil.queryForObject("CUSTOMIZE.update_lnsdiscountaccount_sub", map, TblLnsdiscountaccount.class);
		}
		catch (FabSqlException e)
		{
		...
		//更新后金额为负报错
		if(new FabAmount(lnsdiscountaccount.getBalance().doubleValue()).isNegative()){
			throw new FabException("LNS070");
		}
		...
		if(!VarChecker.isEmpty(receiptNo)){
			...
			lnsAcctInfo.setMerchantNo(lnsdiscountaccount.getCustomid());
		...
		if("1".equals(lnsdiscountaccount.getCusttype())||"PERSON".equals(lnsdiscountaccount.getCusttype())){
			...
			reserv1 = lnsdiscountaccount.getCustomid();
		}else{
			reserv1 = "";
		}
		FundInvest	fundInvest = new FundInvest("", "", channelType, fundChannel, outSerialNo);
		//写事件
		eventProvider.createEvent(ConstantDeclare.EVENT.PREDNTBOFF, amt, lnsAcctInfo, null, 
				fundInvest, chargeType, ctx,
				//预收和保理客户类型统一放至备用5方便发送事件时转换处理
				//转json存入Tunneldata字段 
				reserv1, "","","", lnsdiscountaccount.getName());
	}
