...
public class Tp472005 extends ServiceTemplate {
	...
	protected void run() throws Exception {
		...
		if( lns209.getLoanAgreement().getFeeAgreement().isEmpty() )
//		if("473004".equals(lns219.getTblLnsinterface().getTrancode()))
		{
		...
		lns243.setAcctNo(lns219.getTblLnsinterface().getAcctno());
		lns243.setLoanAgreement(lns209.getLoanAgreement());
		lns243.setTxseqno(lns205.getTxseqno());//子序号
		trigger(lns243);// 费用冲销
		lns103.setLnsinterface(lns219.getTblLnsinterface());
		lns103.setLoanAgreement(lns205.getLoanAgreement());
		lns103.setTxseqno(lns243.getTxseqno());
...
//				DbAccessUtil.execute("CUSTOMIZE.delete_lnsaccountlist", param);
				...
				/* 删除管理费表中该借据号的数据 */
//				DbAccessUtil.execute("CUSTOMIZE.delete_feemanagereg", param);
				/* 删除主文件拓展表中该借据号的数据 */
