...
public  class SettleInterestPrinDintSupporter extends SettleInterestSupporter{  
	@Override
	public  LnsBill settleInterest(LoanAgreement loanAgreement,LnsBill hisBill,String repayDate)
	{
		...
		if(!VarChecker.isEmpty(loanAgreement.getInterestAgreement().getCapRate())){
			...
			if(!VarChecker.isEmpty(capDint)){
				loanAgreement.getBasicExtension().getCalOldDintComein().selfAdd(capDint);
			}else{
				loanAgreement.getBasicExtension().getCalOldDintComein().selfAdd(new FabAmount(capDintDec.doubleValue()));
			}
			//计算已计未结金额
			//账本开始日到更新日天数*日利息
...
//		}
		//封顶计息老数据特殊处理
		if(!VarChecker.isEmpty(loanAgreement.getInterestAgreement().getCapRate())){
			intDec = getOldDintAmt(intDec, loanAgreement);
		}
//		loanAgreement.getBasicExtension().getCalDintComein().selfAdd(new FabAmount(intDec.doubleValue()));
	...
	//计算罚息
	public BigDecimal getOldDintAmt(BigDecimal intDec,LoanAgreement loanAgreement) {
		
		if(intDec.doubleValue() > loanAgreement.getBasicExtension().getCalOldDintComein().getVal()){
			BigDecimal result = BigDecimal.valueOf(loanAgreement.getBasicExtension().getCalOldDintComein().getVal());
			loanAgreement.getBasicExtension().setCalOldDintComein(new FabAmount(0.0));
			return result;
		}else{
			loanAgreement.getBasicExtension().getCalOldDintComein().selfSub(new FabAmount(intDec.doubleValue()));
			return intDec;
		}
