 ...
 * @exception
 */
public  class TransferIntNormalToOverduSupporter extends TransferSupporter{  
	@Autowired
	...
	public void transfer(LnsBill lnsBill, TranCtx ctx, LoanAgreement loanAgreement, String statusbDate) throws FabException{
		...
		LnsAcctInfo normal = new LnsAcctInfo(loanAgreement.getContract().getReceiptNo(),
											 ...
											 new FabCurrency());
		normal.setCancelFlag(lnsBill.getCancelFlag());
		LnsAcctInfo overdue = new LnsAcctInfo(loanAgreement.getContract().getReceiptNo(),
											  lnsBill.getBillType(), 
											  ConstantDeclare.LOANSTATUS.LOANSTATUS_OVERDU, 
											  new FabCurrency());
		overdue.setCancelFlag(lnsBill.getCancelFlag());
		sub.operate(normal, null, lnsBill.getBillBal(), fundInvest, ConstantDeclare.BRIEFCODE.LXZL, lnsBill.getTranDate().toString(), lnsBill.getSerSeqno(), lnsBill.getTxSeq(), ctx);
		add.operate(overdue, null, lnsBill.getBillBal(), fundInvest, ConstantDeclare.BRIEFCODE.LXZL, lnsBill.getTranDate().toString(), lnsBill.getSerSeqno(), lnsBill.getTxSeq(), ctx);
