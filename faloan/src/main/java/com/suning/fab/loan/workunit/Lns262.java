...
@Scope("prototype")
@Repository
public class Lns262 extends WorkUnit {
    ...
    public void run() throws Exception {
        ...
        if (VarChecker.asList("BAK").contains(operation)) {
            try {
                ...
                if (null == lnsinterface) {
                    // 如果没有查询到正向交易的幂等信息，表示正向交易未成功，冲销交易不需要执行，直接返回
                    return;
                }
        ...
        //预收加款事件
        eventProvider.createEvent("LNDKHKYSTH", amt, lnsAcctInfo, oppAcctInfo,
                fundInvest, "YSTH", ctx, amtList,
