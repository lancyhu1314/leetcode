 ...
 * @since [产品/模块版本] （可选）
 */
public class LnsRepayPlanCalculate implements Comparable<LnsRepayPlanCalculate> {
	...
	private FabAmount noretfee; // 本期应还费用                  
	 ...
	 * 构造函数，用于对自定义变量创建空间
	 */
	public LnsRepayPlanCalculate() {
		noretamt = new FabAmount(0.00);
		noretint = new FabAmount(0.00);
		sumamt = new FabAmount(0.00);
		setNoretfee(new FabAmount(0.00));
		//balance = new FabAmount(0.00) //目前不需要返回剩余本金
	}
	
	
	@Override
	public int compareTo(LnsRepayPlanCalculate arg0) {
		return this.getRepayterm().compareTo(arg0.getRepayterm());
    }
	
	@Override
	public boolean equals(Object obj) {
        if (obj instanceof LnsRepayPlanCalculate) {
        	LnsRepayPlanCalculate lnsRepayPlanCalculate = (LnsRepayPlanCalculate) obj;
