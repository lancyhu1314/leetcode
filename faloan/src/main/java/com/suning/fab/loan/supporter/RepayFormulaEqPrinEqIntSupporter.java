...
@Scope("prototype")
@Repository
public class RepayFormulaEqPrinEqIntSupporter extends RepayFormulaSupporter {

	public RepayFormulaEqPrinEqIntSupporter() {
		super();
	}
// 在父类方法里做个判断20191203
	@Override
...
//		}
	...
	public FabAmount getCurrPrin() {
		...
		{
			if("2412617".equals(loanAgreement.getPrdId()) && !VarChecker.isEmpty(loanAgreement.getContract().getRentAmt())){
				repayAmt.selfAdd(new FabAmount(loanAgreement.getContract().getRentAmt().getVal()/totalPeriods));
			}else if(!VarChecker.isEmpty(loanAgreement.getContract().getFlag1()) && loanAgreement.getContract().getFlag1().contains("A")){
				repayAmt.selfAdd(new FabAmount(loanAgreement.getBasicExtension().getBalBeforeEx().getVal()/(totalPeriods-loanAgreement.getBasicExtension().getExtnums()+1)));
			}else if(!VarChecker.isEmpty(loanAgreement.getBasicExtension().getTermEndDate())) {
				repayAmt.selfAdd(new FabAmount(loanAgreement.getContract().getBalance().getVal()
						/(totalPeriods-this.currentPrinPeriod+1)));

			}else{
				repayAmt.selfAdd(new FabAmount(loanAgreement.getContract().getContractAmt().getVal()/totalPeriods));
			}
	}
