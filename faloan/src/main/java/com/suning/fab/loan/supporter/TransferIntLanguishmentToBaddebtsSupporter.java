 ...
 * @exception
 */
public  class TransferIntLanguishmentToBaddebtsSupporter extends TransferSupporter{  
	@Autowired
	...
	public void transfer(LnsBill lnsBill, TranCtx ctx, LoanAgreement loanAgreement, String statusbDate) throws FabException{
		...
		LnsAcctInfo baddebts = new LnsAcctInfo(loanAgreement.getContract().getReceiptNo(),
											  ...
											  new FabCurrency());
		baddebts.setCancelFlag(lnsBill.getCancelFlag());
		sub.operate(languishment, null, lnsBill.getBillBal(), fundInvest, ConstantDeclare.BRIEFCODE.LXZL, lnsBill.getTranDate().toString(), lnsBill.getSerSeqno(), lnsBill.getTxSeq(), ctx);
		add.operate(baddebts, null, lnsBill.getBillBal(), fundInvest, ConstantDeclare.BRIEFCODE.LXZL, lnsBill.getTranDate().toString(), lnsBill.getSerSeqno(), lnsBill.getTxSeq(), ctx);
