 ...
 * @exception
 */
public  class TransferPrinNormalToOverduSupporter extends TransferSupporter{  
	@Autowired
	...
	public void transfer(LnsBill lnsBill, TranCtx ctx, LoanAgreement loanAgreement, String statusbDate) throws FabException{
		...
		LnsAcctInfo overdue = new LnsAcctInfo(loanAgreement.getContract().getReceiptNo(),
											  ...
											  new FabCurrency());
		sub.operate(normal, null, lnsBill.getBillBal(), fundInvest, ConstantDeclare.BRIEFCODE.BJZL, lnsBill.getTranDate().toString(), lnsBill.getSerSeqno(), lnsBill.getTxSeq(), ctx);
		add.operate(overdue, null, lnsBill.getBillBal(), fundInvest, ConstantDeclare.BRIEFCODE.BJZL, lnsBill.getTranDate().toString(), lnsBill.getSerSeqno(), lnsBill.getTxSeq(), ctx);
