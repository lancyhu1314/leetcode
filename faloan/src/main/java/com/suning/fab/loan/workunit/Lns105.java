...
@Repository
public class Lns105 extends WorkUnit { 
...
//		oldrepayacct  = oldrepayacctno.get("acctno").toString();
		...
		lnsinterface.setBankno(getOutSerialNo());//外部流水号
		//科目号存在billno
...
/*		if(!Date.valueOf(getEndDate()).after(Date.valueOf(ctx.getTranDate())))
		...
		//开本金户抛会计系统事件准备
		/**
		 * 计息本金标志:calcIntFlag1	 	1-剩余 2-当期 3-全部
		 * 20190528|14050183
		 */
		...
		{	
			...
			if( !"473005".equals( ctx.getTranCode()) )
			{
				...
				eventProvider.createEvent(ConstantDeclare.EVENT.LOANGRANTA, contractAmt, lnsAcctInfo, null, openFundInvest, ConstantDeclare.BRIEFCODE.BJFK, ctx);
			}
		...
		else//插单条主文件 前三种按合同余额计息方式
		{
			...
			if( null != pkgList  && pkgList.size() > 0 && "473005".equals(ctx.getTranCode()))
