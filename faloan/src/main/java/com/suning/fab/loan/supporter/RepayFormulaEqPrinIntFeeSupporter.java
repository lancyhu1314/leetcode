...
@Scope("prototype")
@Repository
public class RepayFormulaEqPrinIntFeeSupporter extends RepayFormulaSupporter {


	public RepayFormulaEqPrinIntFeeSupporter() {
		super();
	}
	/**
	 * 计算每期应还本金
	...
	public FabAmount getCurrPrin(){
		FabRate totalTermRate=new FabRate(0.00);
		FabRate feeRate=new FabRate(loanAgreement.getFeeAgreement().getLnsfeeinfos().get(0).getFeerate());//费率
		//根据等额本息公式计算应还本金
		...
		BigDecimal feeAmt=new BigDecimal(days)
				.multiply(feeRate.getDayRate())
				.multiply(new BigDecimal(loanAgreement.getContract().getBalance().getVal()));
		feeAmt = feeAmt.setScale(2, BigDecimal.ROUND_HALF_UP);
		totalTermRate.selfAdd(loanAgreement.getRateAgreement().getNormalRate());
		totalTermRate.selfAdd(feeRate);
		...
		BigDecimal nextPeriodInterest = new BigDecimal(days)
										.multiply(totalTermRate.getDayRate())
										.multiply(BigDecimal.valueOf(loanAgreement.getContract().getBalance().getVal()).subtract(repayAmt).add(interest).add(feeAmt));
		...
		amt.selfAdd(repayAmt.subtract(interest).subtract(feeAmt).doubleValue());
	...
	private boolean isDelayRepayPeriod(String endDate) {
...
//				String periodIntStartDate   = calFeeCurrPeriodStartDate(lnsfeeinfo);
//				//计费基数*年费率*期限天数/360天
...
//	}
	...
	public FabAmount getCurrFee(LnsBill feeBill, TblLnsfeeinfo lnsfeeinfo, LoanAgreement la)
	{
		String startDate   = calFeeCurrPeriodStartDate(lnsfeeinfo);
		String endDate=feeBill.getEndDate();
		FabRate feeRate=new FabRate(lnsfeeinfo.getFeerate());
		...
		int overDays = CalendarUtil.actualDaysBetween(loanAgreement.getContract().getRepayPrinDate(), startDate);
		//计算每天利息并四舍五入
		BigDecimal overInterest =  feeRate.getDayRate()
				.multiply(new BigDecimal(loanAgreement.getContract().getBalance().getVal()))
				.setScale(2, BigDecimal.ROUND_HALF_UP);
		//乘以天数
		overInterest = overInterest.multiply(new BigDecimal(overDays));
